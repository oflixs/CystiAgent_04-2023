...
public class Pig implements Steppable
{
    ...
    public Boolean pastSeropositive   = false;
    ...
    public Pig(SimState pstate, Household ph, Boolean pinfDIA, Boolean pinfect)
    {
        ...
        if(!infectDIA)
        {
            ...
            //sim.test = sim.test + homeRange;
            //sim.testCount++;
            //System.out.println("Pig home range mean " + sim.test/sim.testCount);
    ...
    public void step(SimState state)
    {
        ...
        if(pastNumCysts == 0 && numCysts > 0)household.nInfPigs++;
        pastNumCysts = numCysts;

        if(!pastSeropositive && seropositive)household.nPosPigs++;
        pastSeropositive = seropositive;
    ...
    public  void infectPigs()
    {
        ...
        if(numContaminatedSites > 0)
        {
            ...
            if(eggs > 0)
            {
                ...
                //doses of TS contamination to which the pig was exposed this timeStep
                totExpDoses = totExpDoses + sim.seroConvertPtoEFact  * factSero;
            }
            ...
            if(proglottid > 0)
            {
                ...
                //System.out.println("Fact sero proglottid: " + factSero);
                double fact = (double)irand * factSero;
                ...
                //doses of TS contamination to which the pig was exposed this timeStep
                totExpDoses = totExpDoses + factSero;
            }
        }
        ...
        //seroconversion calculation
        if(!seropositiveForExposure && seroConversionTimer < 0)
        {
            rand = state.random.nextDouble();

            double responseProb = 1 - Math.exp(neg * sim.seroConvert * totExpDoses);

            //System.out.println("Dose resposnse Prob: " + responseProb);
...
public void immunityCCystsDegeneration()
{
    ...
    public void gestation()
    {
        ...
        if(pregnant)
        {
            ...
            for(int i = 0; i < nPiglets; i++)
            {
                ...
                if(household.pigs.size() > 1.1 * (double)household.targetNumOfPigs)break;
            }
    ...
    public void getSlaughterAge(String what)
    {
        ...
        double sa = (sim.slaughterAgeMean + state.random.nextGaussian() * sim.slaughterAgeSd);
        ...
        //System.out.println("Pig age " + age);
        //System.out.println("Pig slaughter age " + slaughterAge/sim.weeksInAMonth);

        //if(ageFact == 1.0)
        //{
        //    sim.test = sim.test + slaughterAge/sim.weeksInAMonth;
        //    sim.testCount++;
        //    System.out.println("Pig slaughter age mean " + sim.test/sim.testCount);
        //}
