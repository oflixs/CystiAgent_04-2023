...
public class PigsGenerator implements Steppable
{
    ...
    public void setBreedingSows()
    {
        ...
        for(int i = 0; i < sim.householdsBag.size(); i++)
        {
            ...
            for(int j = 0; j < hh.pigs.size(); j++)
            {
                ...
                pig.breedingSow = false;

                pig.pregnant = false;

                pig.gestationTimer = 0; 
            }
        }
        ...
        for(int i = 0; i < sim.householdsBag.size(); i++)
        {
            ...
            for(int j = 0; j < hh.pigs.size(); j++)
            {
                ...
                if(pig.gender.equals("female"))
                {
                    ...
                    pig.getSlaughterAge("bredingSow");
    ...
    public Pig birthPig(Pig mother)
    {
        ...
        assignPigletsSeropositivity(pig, mother);

        return pig;
    }

    //===============================================
    public void assignPigletsSeropositivity(Pig pig, Pig mother)
    {
        if(sim.seroConvMethod.equals("4parsPigletsPigs"))
        {
            ...
            if(rand < sim.propPigletsMaternalProtection)
            {
                ...
                //if(sim.extendedOutput)System.out.println("Period of maternal antibodies persistence: " +  baseTime);
            }
        }
        else if(sim.seroConvMethod.equals("4monthsPrev"))
        {
            double rand = state.random.nextDouble();
            if(rand < sim.propPigletsMaternalProtection  && mother.numCysts > 0)pig.maternalAntibodiesPersistenceTimer = 14;
        }
