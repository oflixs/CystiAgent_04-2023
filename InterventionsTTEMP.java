...

{
    ...
    public void calculateSeroStats()
    {
        ...
        if(pigsSeroprevStats != 0.0)sim.interSeroPrevalencePigsRounds.set(sim.roundSero, (double)pigsSeroprev/(double)pigsSeroprevStats);
        else sim.interSeroPrevalencePigsRounds.set(sim.roundSero, 0.0);

        if(pigsCohortsSeroprevStats != 0.0) sim.interSeroPrevalencePigsCohortsRounds.set(sim.roundSero, (double)pigsCohortsSeroprev/(double)pigsCohortsSeroprevStats);
        else sim.interSeroPrevalencePigsCohortsRounds.set(sim.roundSero, 0.0);

        if(pigletsSeroprevStats != 0.0)sim.interSeroPrevalencePigletsRounds.set(sim.roundSero, (double)pigletsSeroprev/(double)pigletsSeroprevStats);
        else sim.interSeroPrevalencePigletsRounds.set(sim.roundSero, 0.0);

        if(pigsSeroincStats != 0.0)sim.interSeroIncidencePigsRounds.set(sim.roundSero, (double)pigsSeroinc/(double)pigsSeroincStats);
...
rows:             
    ...
    public void printResume()
    {
        ...
        System.out.println(sim.villageName + " perSeroprevalence adult pigs round 1: " + perAdult);
