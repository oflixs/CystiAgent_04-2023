...
public class ReadShp  
{
    ...
    //====================================================
    public void readLandCover()
    {
        System.out.println("---- " + sim.villageName + " reading land cover shp");

        rootDir = sim.simW.rootDir + "inputData/landCoverData/" + sim.simName + "/";
        ...
        if(sim.landCoverAgriculturalLand)
        {
            ...
            file = sim.villageName + "_agriculturalLand";
            ...
            URL fileDbf = getURL(file + ".dbf");
            URL fileShp = getURL(file + ".shp");

            //reading the file -----------------------------
            try{
                ShapeFileImporter.read(fileShp, fileDbf, sim.agriculturalLand);
                //ShapeFileImporter.read(fileShp, homes);//this was the 
                //method for geoMason ver < 1.6
            }catch (Exception e){//Catch exception if any
                System.err.println("Error: " + e.getMessage());
                System.err.println("File: " + file);
            ...
            Envelope mbr = sim.agriculturalLand.getMBR();
            //sim.globalMBR.expandToInclude(mbr);
            sim.globalMBR = mbr;
            ...
            Bag bag = sim.agriculturalLand.getGeometries();
            System.out.println(sim.villageName  + " Num geometries in agricultural land shp: " + bag.size());
        }
        ...
        if(sim.landCoverIrrigationChannels)
        {
            ...
            file = sim.villageName + "_irrigationChannels";
            ...
            fileDbf = getURL(file + ".dbf");
            fileShp = getURL(file + ".shp");

            //reading the file -----------------------------
            try{
                ShapeFileImporter.read(fileShp, fileDbf, sim.irrigationChannels);
                //ShapeFileImporter.read(fileShp, homes);//this was the 
                //method for geoMason ver < 1.6
            }catch (Exception e){//Catch exception if any
                System.err.println("Error: " + e.getMessage());
            ...
            mbr = sim.irrigationChannels.getMBR();
            if(sim.globalMBR != null)sim.globalMBR.expandToInclude(mbr);
            else sim.globalMBR = mbr;
            ...
            */
            ...
            Bag bag = sim.irrigationChannels.getGeometries();
            System.out.println(sim.villageName  + " Num geometries in irrigationChannels  shp: " + bag.size());

            //-----------------------------------
            dir  = rootDir;
            file = "";

            file = sim.villageName + "_irrigationChannelsPolygon";
            ...
            //reading the file -----------------------------
            try{
                ShapeFileImporter.read(fileShp, fileDbf, sim.irrigationChannelsPolygon);
                //ShapeFileImporter.read(fileShp, homes);//this was the 
                //method for geoMason ver < 1.6
            }catch (Exception e){//Catch exception if any
                System.err.println("Error: " + e.getMessage());
            ...
            mbr = sim.irrigationChannelsPolygon.getMBR();
            if(sim.globalMBR != null)sim.globalMBR.expandToInclude(mbr);
            else sim.globalMBR = mbr;
