...

{
    ...
    public InterventionsR01(SimState pstate)
    {
        ...
        //pitxi
        //sim.village.interventionType = "Mass Trt";
               ...
               )
        {
            ...
            intTimeDouble = (sim.weeksInAMonth * 6);
        ...
        {
            if(sim.extendedOutput)System.out.println("wrong R01 intervention type");
            System.exit(0);
        }
        ...
        getHousesFinalLinesFromFile();
        processHousesFinal();

        //read final mass treatment humans data
        //getFinalHumanMassTreat();
        ...
        if(sim.extendedOutput)System.out.println(sim.villageName + " ring radius: " + sim.ringSize); 
        //
        // System.exit(0);
    ...
    public void step(SimState pstate)
    {
        ...
        if(((now % sim.nPrint) == 0 || sim.nPrint == 1) && !sim.burnin)
        {
            ...
            //System.out.println("Writing to Xls output");
        }
          ...
          )
        {
            ...
            if(sim.extendedOutput)System.out.println(sim.villageName + " R01 " + sim.village.interventionType  + " interventions are starting now:" + time);
        ...
        if(startInt && !stopInt)
        {
            ...
            if(Math.round((intCounter % intTimeDouble)) == 0)
            {
                ...
                else if(sim.village.interventionType.equals("Ring Trt")
                        ||  sim.village.interventionType.equals("Ring Trt (P)"))
                {
                    tongueInspectRound();
                    ...
                    if(sim.village.interventionType.equals("Ring Trt (P)"))
                    {
                        ringTreatP();    
                    }
                }
                        ...
                        ||  sim.village.interventionType.equals("Ring Scr (P)"))
                {
                    ...
                    if(sim.village.interventionType.equals("Ring Scr (P)"))
                    {
                        ringTreatP();    
                    }
                }
        ...
        if(startSero)
        {
            ...
            if(Math.round(intCounterSero % intTimeSeroDouble) == 0)
            {
                ...
                if(sim.roundSero <= sim.seroNRounds)
                {
                    ...
                    {
                        //obs and sim tongue positive prev
                        massTreatP();
                    }
    ...
    public void calculateSeroStats()
    {
        ...
        if(pigsSeroprevStats != 0) sim.interSeroPrevalencePigsRounds.set(sim.roundSero, (double)pigsSeroprev/(double)pigsSeroprevStats);
        else sim.interSeroPrevalencePigsRounds.set(sim.roundSero, 0.0);

        if(pigsCohortsSeroprevStats != 0)sim.interSeroPrevalencePigsCohortsRounds.set(sim.roundSero, (double)pigsCohortsSeroprev/(double)pigsCohortsSeroprevStats);
        else sim.interSeroPrevalencePigsCohortsRounds.set(sim.roundSero, 0.0);

        if(pigletsSeroprevStats !=0)sim.interSeroPrevalencePigletsRounds.set(sim.roundSero, (double)pigletsSeroprev/(double)pigletsSeroprevStats);
        else sim.interSeroPrevalencePigletsRounds.set(sim.roundSero, 0.0);

        if(pigsSeroincStats != 0)sim.interSeroIncidencePigsRounds.set(sim.roundSero, (double)pigsSeroinc/(double)pigsSeroincStats);
        else sim.interSeroIncidencePigsRounds.set(sim.roundSero, 0.0);
    ...
    public void resetSim()
    {
        ...
        if(sim.simW.allVillagesInOneSim)sim.deterministicIndividualsAllocationFile = sim.village.getDetermIndAllFileJoined(); 
        ...
        //Patch to calculate seroInc standard deviation
        //a long journey.....
        List<Double> tmp = new ArrayList<Double>(sim.interSeroIncidencePigsRounds);
        sim.interSeroIncidencePigsRoundsValues.add(tmp);
    }
    ...
    {
        if(sim.extendedOutput)System.out.println(sim.villageName + " ");
        if(sim.extendedOutput)System.out.println(sim.villageName + " =mtp=mtp=mtp=mtp=mtp=mtp=mtp=mtp=mtp=mtp=");
    ...
    {
        if(sim.extendedOutput)System.out.println(sim.villageName + " ");
        if(sim.extendedOutput)System.out.println(sim.villageName + " =mth=mth=mth=mth=mth=mth=mth=mth=mth=mth=");
    ...
    {
        if(sim.extendedOutput)System.out.println("");
        if(sim.extendedOutput)System.out.println(sim.villageName + " -rscrH-rscrH-rscrH-rscrH-rscrH-rscrH-----");
    ...
    {
        if(sim.extendedOutput)System.out.println("");
        if(sim.extendedOutput)System.out.println("-rtrtH-rtrtH-rtrtH-rtrtH-rtrtH-rtrtH-----");
    ...
    {
        if(sim.extendedOutput)System.out.println(" ");
        if(sim.extendedOutput)System.out.println("-rtrtP-rtrtP-rtrtP-rtrtP-rtrtP-rtrtP-----");
...
rows:             
    ...
    public void processLineMT(List<String> line)
    {
        ...
        for(int roundRead = 1; roundRead < (sim.intRounds + 2); roundRead++)
        {
            if(roundRead == 1)
            {
                ...
                if(!tmp.equals(".") && !tmp.equals(""))
                {
                    ...
                    try
                    {
                        cal1.setTime(dateFormat.parse(tmp));
                    }
            ...
            else if(roundRead == 2)
            {
                ...
                if(!tmp.equals(".") && !tmp.equals(""))
                {
                    ...
                    try
                    {
                        cal2.setTime(dateFormat.parse(tmp));
                    }
                    catch (ParseException e)
                    {
                        System.out.println(e);
            ...
            else if(roundRead == 3)
            {
                ...
                if(!tmp.equals(".") && !tmp.equals(""))
                {
                    ...
                    try
                    {
                        cal3.setTime(dateFormat.parse(tmp));
                    }
                    catch (ParseException e)
                    {
                        System.out.println(e);
            ...
            else if(roundRead == 4)
            {
                ...
                if(!tmp.equals(".") && !tmp.equals(""))
                {
                    ...
                    try
                    {
                        cal4.setTime(dateFormat.parse(tmp));
                    }
                    catch (ParseException e)
                    {
                        System.out.println(e);
        ...
        tmpList.add(cal5);
        //tmpList.add(cal6);
        interventionsDateInd.put(ID, tmpList);
...
rows:             
    ...
    public void writeStatsToFile()
    {
        ...
        String tmp = "interventionsStats_" + sim.outFilesStamp + ".xls";

        String fileName = sim.outDirSims;
        fileName = fileName.concat(tmp);

        if(sim.extendedOutput)System.out.println(sim.villageName + " outFile: " + fileName);
    ...
    public void getFinalHTandPC()
    {
        ...
        for(int i = 0; i < sim.humansBag.size(); i++)
        {
            ...
            if(h.age <= (int)Math.round(2 * sim.weeksInAYear))continue;
        ...
        if(sim.extendedOutput)System.out.println("Num humans with taeniasis: " + statsHT + " Num tot humans: " + statsHTNorm);
    ...
    //====================================================
    public void writeSeroIncValuestoXls(HSSFWorkbook wb)
    {
        //erase_file();
        ...
        sheet = wb.getSheet("seroIncValues");
        //If the sheet !exists a new sheet is created --------- 
        if(sheet == null)sheet = wb.createSheet("seroIncValues");
        ...
        cell = row.createCell(cellnum++);
        cell.setCellValue((String)"Here the values of sero incidence obtained in the 7 intervention rounds of several independent runs of R01 village");

        row = sheet.createRow(lastRow++);
        cellnum = 0;

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)("1"));

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)("2"));

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)("3"));

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)("4"));

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)("5"));

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)("6"));

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)("7"));


        for(int i = 0; i < sim.interSeroIncidencePigsRoundsValues.size(); i++)
        {
            row = sheet.createRow(lastRow++);
            cellnum = 0;

            List<Double> tmp = new ArrayList<Double>(sim.interSeroIncidencePigsRoundsValues.get(i));

            for(int j = 1; j < tmp.size(); j++)
            {
                cell = row.createCell(cellnum++);
                cell.setCellValue((Double)tmp.get(j));
            }
        }
    }
    ...
    */
    ...
    //====================================================
    public void getFinalHumanMassTreat()
    {
        if(sim.extendedOutput)System.out.println (" ");
        if(sim.extendedOutput)System.out.println ("----------------------------------------------------");
        if(sim.extendedOutput)System.out.println ("----------------------------------------------------");
        if(sim.extendedOutput)System.out.println ("---- Reading the final HT data ----");
        String inputFile = "";
        String sheetName = "";

        inputFile = "./inputData/interventions/R01/R01_AllTaeniasis_StudyEnd.xlsx";
        sheetName = "Sheet1";
        if(sim.extendedOutput)System.out.println ("Survey final HT input file: " + inputFile);
        ...
        try{
            ...
            boolean startRead = false;
            boolean stopRead = false;
            List <List<String>>houseLines = new ArrayList<List<String>>();
            List<String> line = new ArrayList<String>();
rows:             
            for(Row row : sheet)
            { 
                ...
                int villId = Integer.parseInt(line.get(1));
                int hId = Integer.parseInt(line.get(2));
                String vill = line.get(1);
                ...
                {
                    //if(sim.extendedOutput)System.out.println(vill);
                    //if(sim.extendedOutput)System.out.println(hId);

                    Household hh = sim.householdsGen.getHouseholdByshpId(hId);
            ...
            double numHumans = 0.0;
            if(sim.village.interventionType.equals("Ring Trt")
                    ||  sim.village.interventionType.equals("Ring Trt (P)")
              )
            {
                numHumans = (double)(statsPart.get(8) + statsNoPart.get(8));
            }
            else if(sim.village.interventionType.equals("Ring Scr")
                    ||  sim.village.interventionType.equals("Ring Scr (P)")
                   )
            ...
            {
                numHumans = (double)(statsPart.get(6) + statsNoPart.get(6));
            }


            obsPrevHTFinal = (double)numInfected/numHumans;
            if(sim.extendedOutput)System.out.println(sim.villageName + " R01 final round HT prevalence num humans: " + numHumans + " num infected humans: " + numInfected + " num humans: " + numHumans);
            if(sim.extendedOutput)System.out.println(sim.villageName + " R01 final round HT prevalence: " + obsPrevHTFinal);
    ...
    //====================================================
    public void processHousesFinal()
    {
        String oldHouse = "";
        String newHouse = "";
        int stats = 0;
        ...
        for(int houseNum : housesLinesMap.keySet())
        {
            ...
            int hId = Integer.parseInt(line.get(3));
            ...
            if(hh == null)
            {
                if(sim.extendedOutput)System.out.println("household not found: " + hId);
                continue;
            }

            //if(sim.extendedOutput)System.out.println(" ");
            //if(sim.extendedOutput)System.out.println("----------------------------");
            //if(sim.extendedOutput)System.out.println("Household: " + hId);

            if(!line.get(11).equals("."))
            {
                tmp = Integer.parseInt(line.get(11));
                if(tmp != hh.sshhValue)
                {
                    if(sim.extendedOutput)System.out.println("household " + hId + " differs in sshhValue  original: " + hh.sshhValue + " new: " + tmp);
                    if(sim.extendedOutput)System.out.println("If village read from file this is not necesarly true");
                }
            }

            if(!line.get(12).equals("."))
            {
                tmp = Integer.parseInt(line.get(12));
                if(tmp != hh.eliminahecValue)if(sim.extendedOutput)
                {
                    System.out.println("household " + hId + " differs in eliminahec  original: " + hh.eliminahecValue + " new: " + tmp);
                    if(sim.extendedOutput)System.out.println("If village read from file this is not necesarly true");
                }
            }
            ...
            {
                line = houseLines.get(i);

                numHumans++;
                numHumansVillage++;

                String ID = line.get(33);

                if(!line.get(48).contains("02"))//excludes age < 2 from participation calculations
                {
                    tmpS = line.get(47);
                    if(tmpS.equals(".") || tmpS.equals("0"))numNoPart++;
                    else 
                    {

                        if(line.get(83).equals("POSITIVO"))numPositive++;
                        else numNegative++;

                        numPart++;

                        if(interventionsDateInd.containsKey(ID));
                        {
                            //if(sim.extendedOutput)System.out.println ("-------");
                            //if(sim.extendedOutput)System.out.println ("Human: " + ID);

                            tmpC = line.get(46); 

                            Calendar cal = null;
                            if(!tmpC.equals(".") && !tmpC.equals(""))
                            {
                                //System.out.println("first date: " + tmpC);
                                cal = Calendar.getInstance();
                                try
                                {
                                    cal.setTime(dateFormat.parse(tmpC));
                                }
                                catch (ParseException e)
                                {
                                    System.out.println(e);
                            ...
                            if(cal != null)
                            {
                                ...
                                List<Calendar> tmpList = interventionsDateInd.get(ID);
                                //if(sim.extendedOutput)System.out.println ("interventions dates: " + tmpList);
                                if(tmpList != null)
                                {
                                    tmpList.add(cal);
                                    interventionsDateInd.put(ID, tmpList);
                                      ...
                                      )
                                    {
                                        ...
                                        {
                                            //if(sim.extendedOutput)System.out.println ("interventions dates: " 
                                            //        + tmpList.get(4).get(Calendar.WEEK_OF_YEAR) + " "
                                            //        + tmpList.get(5).get(Calendar.WEEK_OF_YEAR) + " "
                                            //        );

                                            weekDiff = tmpList.get(5).get(Calendar.WEEK_OF_YEAR) - tmpList.get(4).get(Calendar.WEEK_OF_YEAR);
                                        }
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            //if(sim.extendedOutput)System.out.println ("interventions dates: " 
                                            //        + tmpList.get(6).get(Calendar.WEEK_OF_YEAR) + " "
                                            //        + tmpList.get(7).get(Calendar.WEEK_OF_YEAR) + " "
                                            //        );

                                            weekDiff = tmpList.get(7).get(Calendar.WEEK_OF_YEAR) - tmpList.get(6).get(Calendar.WEEK_OF_YEAR);
                                        }
                                    ...
                                    //if(sim.extendedOutput)System.out.println ("weeksDiff: " + weekDiff);
                                    if(weekDiff != 0)
                                    {
                                        weeksDiffFinalRound = weeksDiffFinalRound + weekDiff;
                                        statsWeeksDiffFinalRound++;
            ...
            tmp = hh.humans.size();
            //if(tmp != numHumans)if(sim.extendedOutput)System.out.println("household differs in numHumans  original: " + tmp + " new: " + numHumans);
        ...
        if(sim.extendedOutput)System.out.println (" ");
        if(sim.extendedOutput)System.out.println ("---------------------");
        if(sim.extendedOutput)System.out.println ("Intervention stats");
        if(sim.extendedOutput)System.out.println ("Original number of people village: " + sim.humansBag.size()+  " new village size: " + numHumansVillage);
        if(sim.extendedOutput)System.out.println ("Final mass treatment: individual enrolled: " + (numPart + numNoPart));
        if(sim.extendedOutput)System.out.println ("Final mass treatment: individual partecipated: " + numPart);
        if(sim.extendedOutput)System.out.println ("Final mass treatment: individual NOT partecipated: " + numNoPart);
        if(sim.extendedOutput)System.out.println ("Final mass treatment: participation rate: " + (double)numPart/(double)(numPart + numNoPart));

        if(sim.extendedOutput)System.out.println (" ");
        if(sim.extendedOutput)System.out.println ("Final mass treat - individuals Positive: " + numPositive);
        if(sim.extendedOutput)System.out.println ("Final mass treat - individuals Negative: " + numNegative);
        if(sim.extendedOutput)System.out.println ("Final mass treat - HT prevalence: " + (double)numPositive/(double)(numPositive + numNegative));

        obsPrevHTFinal = (double)numPositive/(double)(numPositive + numNegative);

        if(sim.extendedOutput)System.out.println (" ");
        oldFinalIntTimer = (int)Math.round((double)weeksDiffFinalRound/(double)statsWeeksDiffFinalRound);
        finalIntTimer = Integer.valueOf(oldFinalIntTimer);
        if(sim.extendedOutput)System.out.println("finalInTimer: " + finalIntTimer + " weeks");
        if(oldFinalIntTimer == 0)
        {
            if(sim.extendedOutput)System.out.println("finalInTimer is 0");
            if(sim.extendedOutput)System.out.println("this is a problem because tihs is the interval between last humans intervention");
            if(sim.extendedOutput)System.out.println("and the final human mass treatment");
            if(sim.extendedOutput)System.out.println("this interval is assumed to be the average of the value over the remaining villages in the same trial arm");
    ...
    //====================================================
    public void getHousesFinalLinesFromFile()
    {
        if(sim.extendedOutput)System.out.println (" ");
        if(sim.extendedOutput)System.out.println ("----------------------------------------------------");
        if(sim.extendedOutput)System.out.println ("----------------------------------------------------");
        if(sim.extendedOutput)System.out.println ("---- Reading the final HT mass treatment data ----");
        String inputFile = "";
        String sheetName = "";

        inputFile = "./inputData/interventions/R01/R01_Muestreo_Masivo_Final_072017.xlsx";
        sheetName = "Data_R01";
        if(sim.extendedOutput)System.out.println ("Final mass treatment input file: " + inputFile);
        //
...
rows:             
        ...
        //System.out.println("houseLinesMap size: " + housesLinesMap);
