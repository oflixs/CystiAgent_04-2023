...
//----------------------------------------------------
public class InterventionsAfricanSwineFever implements Steppable

{
    ...
    public double fractPigsKilledByThePandemic = 0.75;
    //public double pandemicDuration = 60;//in months
    public double pandemicDuration = 40;//in months
    public double equilDuration = 200;//in months
    public double equilDurationInWeeks = 200;//in months
    public double pandemicDurationInWeeks = 0;//in months
    public int timeIntRoundsInt = 4;//in weeks
    public int timeIntRoundsSero = 4;//in weeks
    public double equilNRounds = 0;
                                               //
    public int initPigsPop = 0;

    //====================================================
    public InterventionsAfricanSwineFever(SimState pstate)
    {
        //this is to simulate the same time scheduling of
        //pigs seroscreening of for an AfricanSwineFever pandemy scenario
        //in 5 year 85% of pigs are killed by the pandemic
        //
        ...
        sim.preInterventionsNumStep = 104;
        sim.postInterventionsNumStep = 104;

        sim.schedule.scheduleRepeating(1.0, 14, this);

        //African interventions ------------------

        intTimeSeroDouble = (sim.weeksInAMonth * timeIntRoundsInt);
        intTimeSero = (int)Math.round(sim.weeksInAMonth * timeIntRoundsInt);
        intTimeSeroMonths = timeIntRoundsInt;

        //number of pig serology samples
        pandemicDurationInWeeks = (int)Math.round(pandemicDuration * sim.weeksInAMonth);
        equilDurationInWeeks = (int)Math.round(equilDuration * sim.weeksInAMonth);
        sim.intRounds = (int)Math.round((pandemicDurationInWeeks + equilDurationInWeeks)/ intTimeSero);

        equilNRounds = (int)Math.round((equilDurationInWeeks)/ intTimeSero);

        //if(sim.extendedOutput)System.out.println("pandemicDurationInWeeks: " + pandemicDurationInWeeks);
        //if(sim.extendedOutput)System.out.println("sim.intRounds: " + sim.intRounds);
        //System.exit(0);

        intTimeDouble = (sim.weeksInAMonth * timeIntRoundsSero);
        intTime = (int)Math.round(sim.weeksInAMonth * timeIntRoundsSero);
        intTimeMonths = timeIntRoundsSero;
        //num rounds 
        sim.seroNRounds = (int)Math.round(sim.intRounds * 10.0);

        if(sim.extendedOutput)System.out.println("Interv n rounds: " + sim.intRounds);
        if(sim.extendedOutput)System.out.println("Equil n rounds: " + equilNRounds);
        ...
        if(sim.extendedOutput)System.out.println(sim.villageName + " Intervention type: " + sim.interventionsType);
        ...
        for(int i = 0; i < (sim.seroNRounds + 1); i++)
        {
            ...
            sim.interHTRounds.add(0.0);
            sim.interPCRounds.add(0.0);
            ...
            sim.avgHTRounds.add(0.0);
            sim.avgPCRounds.add(0.0);

            sim.avgInterNumPigsRounds.add(0.0);
            sim.avgInterNumPigsInfectedRounds.add(0.0);
    ...
    public void step(SimState pstate)
    {
        ...
        if(((runningRound > sim.intRounds) && (sim.roundSero > sim.seroNRounds))
                && !stopInt)
        {
            stopInt = true;
            intCounter = 0;
        ...
        if(startInt && !stopInt)
        {
            ...
            //here interventions in each step
            if(runningRound > equilNRounds)
            {
                numWeeksInt++;
                exponentialPigPopDecay();
            }
            //exponentialPigPopDecayByHouseholds();
            //exponentialPigPopDecayByArea();

            if(Math.round((intCounter % intTimeDouble)) == 0)
            {
                //here the interventios in intervention times
    ...
    public void calculateSeroStats()
    {
        ...
        double pigsInfected = 0;
        double pigsInfectedStats = 0;

        double humansInfected = 0;
        double humansInfectedStats = 0;
        ...
        for(int i = 0; i < sim.pigsBag.size(); i++)
        {
            ...
            if(pig.numCysts > 0)pigsInfected++;
            pigsInfectedStats++;
        ...
        if(pigsInfectedStats != 0)sim.interPCRounds.set(sim.roundSero, (double)pigsInfected/(double)pigsInfectedStats);
        else sim.interHTRounds.set(sim.roundSero, 0.0);

        if(humansInfectedStats != 0)sim.interHTRounds.set(sim.roundSero, (double)humansInfected/(double)humansInfectedStats);
        else sim.interPCRounds.set(sim.roundSero, 0.0);
        ...
        newValue = (double)sim.pigsBag.size() + sim.avgInterNumPigsRounds.get(sim.roundSero);
        sim.avgInterNumPigsRounds.set(sim.roundSero, (newValue));

        newValue = (double)pigsInfected + sim.avgInterNumPigsInfectedRounds.get(sim.roundSero);
        sim.avgInterNumPigsInfectedRounds.set(sim.roundSero, (newValue));

        newValue = sim.interPCRounds.get(sim.roundSero) + sim.avgPCRounds.get(sim.roundSero);
        sim.avgPCRounds.set(sim.roundSero, (newValue));

        newValue = sim.interHTRounds.get(sim.roundSero) + sim.avgHTRounds.get(sim.roundSero);
        sim.avgHTRounds.set(sim.roundSero, (newValue));
        ...
        if(sim.extendedOutput)System.out.println(" ");
        if(sim.extendedOutput)System.out.println(sim.villageName + " Num pigs this round: " + sim.pigsBag.size());
        if(sim.extendedOutput)System.out.println(sim.villageName + " Avg. num pigs this round: " + sim.avgInterNumPigsRounds.get(sim.roundSero)/(double)numInterventionsDone);

        if(sim.extendedOutput)System.out.println(" ");
        if(sim.extendedOutput)System.out.println(sim.villageName + " Num pigs infected this round: " + pigsInfected);
        if(sim.extendedOutput)System.out.println(sim.villageName + " Avg. num pigs this round: " + sim.avgInterNumPigsInfectedRounds.get(sim.roundSero)/(double)numInterventionsDone);

        if(sim.extendedOutput)System.out.println(" ");
        if(sim.extendedOutput)System.out.println(sim.villageName + " HT prevalence: " + sim.interHTRounds.get(sim.roundSero));
        if(sim.extendedOutput)System.out.println(sim.villageName + " Avg. PC prevalence: " + sim.avgPCRounds.get(sim.roundSero)/(double)numInterventionsDone);
    ...
    public void resetSim()
    {
        ...
        if(sim.extendedOutput)System.out.println("num adult pigs: " + stats);
        */
...
rows:             
    ...
    public void writeSeroInctoXls(HSSFWorkbook wb)
    {
        ...
        cell = row.createCell(cellnum++);
        cell.setCellValue((String)(sim.villageName + " HT prevalence"));

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)(sim.villageName + " PC prevalence"));

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)(sim.villageName + " num Pigs"));

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)(sim.villageName + " num Pigs Infected"));
        ...
        for(int i = 1; i <= sim.seroNRounds; i++)
        {
            ...
            cell = row.createCell(cellnum++);
            cell.setCellValue((Double)sim.avgHTRounds.get(i)/(double)numInterventionsDone);

            cell = row.createCell(cellnum++);
            cell.setCellValue((Double)sim.avgPCRounds.get(i)/(double)numInterventionsDone);

            cell = row.createCell(cellnum++);
            cell.setCellValue((double)sim.avgInterNumPigsRounds.get(i)/(double)numInterventionsDone);

            cell = row.createCell(cellnum++);
            cell.setCellValue((double)sim.avgInterNumPigsInfectedRounds.get(i)/(double)numInterventionsDone);
        }
    }
    ...
    */
    ...
    public void getInitPigsPop()
    {
        ...
        for(int i = 0; i < sim.householdsBag.size(); i++)
        {
            ...
            initPigsPop = initPigsPop + h.targetNumOfPigs;
        }
    ...
    //====================================================
    public void exponentialPigPopDecay()
    {
        Boolean po = false;

        if(po)if(sim.extendedOutput)System.out.println("Exponential Pig Pop Decay ------");
        //P(t) = P0 * exp(- t/lambda)

        //lambda = - tfinal / ln(fractPigsKilledByThePandemic)
        double lambda = -1.0 * (double)pandemicDurationInWeeks / Math.log((1.0 - fractPigsKilledByThePandemic)); 

        if(po)if(sim.extendedOutput)System.out.println("fractPigsKilledByThePandemic: " + fractPigsKilledByThePandemic);
        if(po)if(sim.extendedOutput)System.out.println("pandemicDurationInWeeks: " + pandemicDurationInWeeks);
        if(po)if(sim.extendedOutput)System.out.println("lambda: " + lambda);

        if(po)if(sim.extendedOutput)System.out.println(" ");

        if(po)if(sim.extendedOutput)System.out.println("numWeeksInt: " + numWeeksInt);

        if(po)if(sim.extendedOutput)System.out.println("runningRound: " + runningRound);
        if(po)if(sim.extendedOutput)System.out.println("sim.intRounds: " + sim.intRounds);

        int numPigsExponentialNow = (int)(initPigsPop * Math.exp(-1.0 * (double)numWeeksInt / lambda));
        int numPigsExponentialFinal = (int)(initPigsPop * Math.exp(-1.0 * (double)pandemicDurationInWeeks / lambda));

        if(po)if(sim.extendedOutput)System.out.println("numPigsExponentialFinal: " + numPigsExponentialFinal);
        if(po)if(sim.extendedOutput)System.out.println("numPigsExponentialNow: " + numPigsExponentialNow);

        if(po)if(sim.extendedOutput)System.out.println("initPigsPop: " + initPigsPop);

        int housesTargetInTheSimNow = 0;
        int numPigsInTheSimNow = 0;
        for(int i = 0; i < sim.householdsBag.size(); i++)
        {
            Household h = (Household)sim.householdsBag.get(i);

            housesTargetInTheSimNow = housesTargetInTheSimNow + h.targetNumOfPigs;
            numPigsInTheSimNow = numPigsInTheSimNow + h.pigs.size();
        }

        if(po)if(sim.extendedOutput)System.out.println("housesTargetInTheSimNow: " + housesTargetInTheSimNow);
        if(po)if(sim.extendedOutput)System.out.println("numPigsInTheSimNow: " + numPigsInTheSimNow);

        int targetDiff = housesTargetInTheSimNow - numPigsExponentialNow;
        int pigsDiff = numPigsInTheSimNow - numPigsExponentialNow;

        if(po)if(sim.extendedOutput)System.out.println("targetDiff: " + targetDiff);
        if(po)if(sim.extendedOutput)System.out.println("pigsDiff: " + pigsDiff);

        if(targetDiff <= 0 && pigsDiff <= 0)return;
        if(numPigsInTheSimNow <= numPigsExponentialFinal)return;
        ...
        for(int i = 0; i < sim.householdsBag.size(); i++)
        {
            ...
            if(h.targetNumOfPigs > 0 && (statsTarget < targetDiff))
            {
                h.targetNumOfPigs--;
                statsTarget++;
            }

            List<Pig> tmpPigs = new ArrayList<Pig>(h.pigs);

            if(statsPigs < pigsDiff && h.pigs.size() > 0)
            {
                for(int j = 0; j < tmpPigs.size(); j++)
                {
                    Pig pig = tmpPigs.get(j);
                    pig.die();
                    statsPigs++;
                    break;
                }
            ...
            if((statsTarget >= targetDiff) && (statsPigs >= pigsDiff))break;
        }

    }

    //====================================================
    public void exponentialPigPopDecayByHouseholds()
    {
        Boolean po = false;
        ...
        for(int i = 0; i < sim.householdsBag.size(); i++)
        {
            ...
            if(h.targetNumOfPigs > 0 && (statsTarget < targetDiff))
            {
                statsTarget = statsTarget + h.targetNumOfPigs;
                h.targetNumOfPigs = 0;
            }
    ...
    public void modifyPigsPop()
    {
        ...
        int numPigsExponentialNow = (int)(initPigsPop * Math.exp(-1.0 * (double)numWeeksInt / lambda));
        int numPigsExponentialFinal = (int)(initPigsPop * Math.exp(-1.0 * (double)pandemicDurationInWeeks / lambda));
        numPigsExponentialNow = numPigsExponentialFinal;
