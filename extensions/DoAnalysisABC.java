...
public class DoAnalysisABC implements Serializable
{
    ...
    HSSFWorkbook ABCIndipWorkbook = null;
    ...
    */
    ...
    public void writeObservables(HSSFWorkbook wb)
    {
        ...
        cell = row.createCell(cellnum);
        cell.setCellValue((String)"Prior distribution uniform");
        ...
        for(String name : observed.keySet())
        {
            ...
            cell = row.createCell(cellnum);
            cell.setCellValue((String)"Weight");
            cellnum++;
        }
        ...
        for(String villageName : abcSingle.villages.keySet())
        {
            ...
            for(String name : observed.keySet())
            {
                ...
                cell = row.createCell(cellnum);
                cell.setCellValue((Double)abcSingle.obsABCValuesWeights.get(village.name).get(name));
                cellnum++;
            }
...
rows:             
    ...
    //====================================================
    public void analysisIndip(String pwhatRead, String pprintRuns, int pNumRunsToBeAccepted)
    {
        numRunsToBeAccepted = pNumRunsToBeAccepted;
        ...
        System.out.println ("===============================================================");
        System.out.println ("---------------------------------------------------------------");
        System.out.println ("ExtsABC Analysis ---- ABC analysis indipendent villages started");
        //rm old files
        ...
        if(whatRead.equals("own"))
        {
            abcSingle.ABCIndipOutFile = abcSingle.ABCDir + "/data/ABCIndipresults_" + abcSingle.nameSuffix  + "_" + ext.computerName + ".xls";
            if(ext.sensitivityAnalysis)abcSingle.ABCIndipOutFile = abcSingle.ABCDir + "/data/resultsIndip_" + abcSingle.nameSuffix  + "_" + ext.computerName + ".xls";
        }
        else if(whatRead.equals("master"))
        {
            abcSingle.ABCIndipOutFile = abcSingle.ABCDir + "/ABCIndipresults_" + abcSingle.nameSuffix  + "_" + "master" + ".xls";
            if(ext.sensitivityAnalysis)abcSingle.ABCIndipOutFile = abcSingle.ABCDir + "/resultsIndip_" + abcSingle.nameSuffix  + "_" + "master" + ".xls";
        }

        ABCIndipWorkbook = new HSSFWorkbook();
        ABCAnalysisReturn = false;
        ...
        */
        ...
        for(String villageName : abcSingle.villages.keySet())
        {
            ...
            writeVillagesResultsIndip(ABCIndipWorkbook, village, sheetName);
            ABCWorkbook.setSheetOrder(sheetName, 0);
        ...
        System.out.println (" ");
        System.out.println ("ExtsABC Analysis ---- end analysis indipendent runs------------");
        System.out.println ("---------------------------------------------------------------");
        System.out.println ("===============================================================");
    ...
    //====================================================
    public void writeToFileIndip()
    {
        System.out.println("Exts Analysis ---- file idipendent villages xls: " + abcSingle.ABCIndipOutFile);
        try {

            FileOutputStream out = 
                new FileOutputStream(new File(abcSingle.ABCIndipOutFile));
            ABCIndipWorkbook.write(out);
            out.close();
            //System.out.println(abcSingle.ABCOutFile);
            System.out.println("Exts Analysis ---- output spreadsheet indipendent villages written successfully.");
    ...
    //====================================================
    public void writeVillagesResultsIndip(HSSFWorkbook wb, Village village, String sheetName)
    {
        HSSFSheet sheet = wb.getSheet(sheetName);
        //If the sheet !exists a new sheet is created --------- 
        if(sheet == null)sheet = wb.createSheet(sheetName);
        ...
        for(String villageName : localPars.keySet())
        {
            if(!villageName.equals(village.name))continue;
            //System.out.println ("villageName: " + villageName);
            HashMap<String, Double> localObs = (HashMap<String, Double>)localPars.get(villageName);
        ...
        long lll = 0;
        HashMap <Long, Double> tmp = new HashMap<Long, Double>();
        for(int jM = 0; jM < limit; jM++)
        {
            tmp = village.distancesList.get(jM);

            for(Long jj : tmp.keySet())
            {
                lll = jj; 
                break;
            }
            ...
            //ABCRun rrr = runsList.get(j);
            ABCRun rrr = abcSingle.runs.get(lll);
            //long lll = rrr.num;
            //System.out.println ("lll: " + lll);
            ...
            */
            ...
            globalPars = rrr.globalParRealizations;
            for(String name : globalPars.keySet())
            {
                cell = row.createCell(cellnum);
                cell.setCellValue((Double)globalPars.get(name));
            ...
            localPars = rrr.localParRealizations;
            for(String villageName : localPars.keySet())
            {
                if(!villageName.equals(village.name))continue;
                HashMap<String, Double> localObs = (HashMap<String, Double>)localPars.get(villageName);
