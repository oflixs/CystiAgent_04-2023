...
public class OutcomesPool implements Serializable
{
    ...
    public DoAnalysisOutcomePoolJoined doAnalysisJoined;

    public final String runFile;
    public final String runFileName;
    public final String templateRunFile;

    //pitxi to allVillageinOneSim
    //public finale String simsDir;
    ...
    public OutcomesPool(Extensions pext, ABCSingleOneRun pabcOne)
    {
        ...
        if(ext.allVillagesInOneSim)
        {
            doAnalysisJoined  = new DoAnalysisOutcomePoolJoined(ext, this);
        }
        else
        {
        ...
        else
        {
            ...
            System.out.println("ExtOut ---- sims dir: " + simsDir);
    ...
    //====================================================
    public void changeFilesNamesJoined()
    {
        String name = "";
        for (int j = 0; j < ext.villagesNames.size(); j++)
        {
            name = ext.villagesNames.get(j);
            if(j == 0)ext.joinedVillageName = name;
            else ext.joinedVillageName = ext.joinedVillageName + "_" + name;
        }
        ext.joinedVillageName = ext.joinedVillageName + "_Joined";

        System.out.println ("Extensions ---- joined village name: " + ext.joinedVillageName);


        ext.villagesNamesJoined = new ArrayList<String>(); 

        ext.villagesNamesJoined.add(ext.joinedVillageName); 


        String villageName = ext.villagesNamesJoined.get(0);
        simsDir = "../outputs/" + ext.simName + "/" + villageName  +  "/sims/";
