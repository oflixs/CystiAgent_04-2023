...
public class ABCCalibrationSingle implements Serializable
{
    ...
    HashMap<String, HashMap<String, Double>> obsABCValuesWeights = new HashMap<String, HashMap<String,Double>>();
    ...
    public List<Double> stageTargetsWeights = new ArrayList<Double>();
    ...
    public int readObjectsRunsRecursively(Boolean onlyNumbers, Boolean doStageResults, String what)
    {
        ...
        if(doStageResults)
        {
            ...
            for(String villageName : villages.keySet())
            {
                ...
                for(String parName : obsABCValues.get(village.name).keySet())
                {
                    ...
                    stageTargetsWeights.add(obsABCValuesWeights.get(village.name).get(parName));
                }
        ...
        for(String fileName: directoryContents) 
        {
            ...
            if(words[0].equals("runsResultsABC") && words[(words.length - 1)].equals("obj"))
            {
                ...
                checkObjFileForInputConsistency(file);
                //System.exit(0);
    ...
    //====================================================
    public void checkObjFileForInputConsistency(File file)
    {
        Boolean printOut = false;
        if(printOut)System.out.println ("ExtsABC ---- checking file: " + file + " for consistency with inputs");
        try {
            FileInputStream fi = new FileInputStream(file);
            ObjectInputStream oi = new ObjectInputStream(fi);
            ...
            while (check){
                ...
                try{
                    ...
                    //------------------------------------
                    if(!run.villagesNames.containsAll(ext.villagesNames) 
                    || !ext.villagesNames.containsAll(run.villagesNames) )
                    {
                        System.out.println ("---------------------------------");
                        System.out.println ("Error: The villages in the extensions input files");
                        System.out.println ("differ from the villages stored in outputfiles");
                        System.out.println ("Villages in input:");
                        System.out.println (ext.villagesNames);
                        System.out.println ("Villages in run:");
                        System.out.println (run.villagesNames);
                        System.exit(0);
                    }

                    List<String> namesRun = run.globalParRealizations.keySet().stream().collect(Collectors.toList());

                    //------------------------------------
                    if(!namesRun.containsAll(ext.paraABCGlobal) 
                    || !ext.paraABCGlobal.containsAll(namesRun) )
                    {
                        System.out.println ("---------------------------------");
                        System.out.println ("Error: The global parameters in the extensions input files");
                        System.out.println ("differ from the global parameters stored in outputfiles");
                        System.out.println ("Global parameters in input:");
                        System.out.println (ext.paraABCGlobal);
                        System.out.println ("Global parameters in run:");
                        System.out.println (namesRun);
                        System.exit(0);
                    }

                    Iterator<String> iterator = run.localParRealizations.keySet().iterator();
                    HashMap<String, Double> tmp = run.localParRealizations.get( iterator.next() );
                    namesRun = tmp.keySet().stream().collect(Collectors.toList());

                    //------------------------------------
                    if(!namesRun.containsAll(ext.paraABCLocal) 
                    || !ext.paraABCLocal.containsAll(namesRun) )
                    {
                        System.out.println ("---------------------------------");
                        System.out.println ("Error: The Local parameters in the extensions input files");
                        System.out.println ("differ from the Local parameters stored in outputfiles");
                        System.out.println ("Local parameters in input:");
                        System.out.println (ext.paraABCLocal);
                        System.out.println ("Local parameters in run:");
                        System.out.println (namesRun);
                        System.exit(0);
                    }


                    for (Map.Entry<String, HashMap<String, Double>> entryRun : run.observed.entrySet()) 
                    {
                        String keyRun = entryRun.getKey();
                        HashMap<String, Double> valueRun = entryRun.getValue();

                        for (Map.Entry<String, HashMap<String, Double>> entrySim : obsABCValues.entrySet()) 
                        {
                            String keySim = entrySim.getKey();
                            HashMap<String, Double> valueSim = entrySim.getValue();

                            if(keySim.equals(keyRun))
                            {
                                //------------------------------------
                                if(!valueSim.equals(valueRun))
                                {
                                    System.out.println ("---------------------------------");
                                    System.out.println ("Error: The observed target values in the extensions input files");
                                    System.out.println ("differ from the observed target values stored in outputfiles");
                                    System.out.println ("observed target values in input:");
                                    System.out.println (valueSim);
                                    System.out.println ("observed target values in run:");
                                    System.out.println (valueRun);
                                    System.exit(0);
                                }
                                break;
                            }
                    ...
                    if(printOut)System.out.println (namesRun);

                    if(printOut)System.out.println (" ");
                    if(printOut)System.out.println ("Run globalParRealizatins: " + run.globalParRealizations);
                    if(printOut)System.out.println (" ");
                    if(printOut)System.out.println ("Run localParRealizatins: " + run.localParRealizations);
                    //if(printOut)System.out.println (" ");
                    //if(printOut)System.out.println ("Run inputs: " + run.inputs);
                    //if(printOut)System.out.println (" ");
                    //if(printOut)System.out.println ("Run inputsCystiHumans: " + run.inputsCystiHumans);
                    if(printOut)System.out.println (" ");
                    if(printOut)System.out.println ("Run obsABCConv: " + run.obsABCConv);
                    if(printOut)System.out.println (" ");
                    if(printOut)System.out.println ("Run results: " + run.results);

                    if(printOut)System.out.println (" ");
                    if(printOut)System.out.println ("Run observed: " + run.observed);
                    if(printOut)System.out.println (" ");
                    if(printOut)System.out.println ("Sim observed: " + obsABCValues);
                    */
        ...
        if(printOut)System.out.println ("ExtsABC ---- checking file: OK");
    }
    ...
    public void readABCParametersRanges(String what)
    {
        ...
        try
        {
            ...
            while ((strLine = br.readLine()) != null)   
            {
                ...
                if(printOut)System.out.println ("strLine: " + strLine);
                ...
                if(printOut)System.out.println ("---- parName: " + parName);
                if(printOut)System.out.println ("----- paraABCLocal: " + ext.paraABCLocal);
                ...
                if(printOut)System.out.println ("----- paraABCGlobal: " + ext.paraABCGlobal);
    ...
    public void readABCTargets()
    {
        ...
        HashMap<String, Double> parValuesWeights = new HashMap<String, Double>();

        try{
            Workbook workbookFile = WorkbookFactory.create(new FileInputStream(ext.fileObsABC));
            System.out.println ("ExtABCSingle ---- target observables file: " + ext.fileObsABC);
...
rows:             
            ...
            { 
                ...
                parValuesWeights = new HashMap<String, Double>();
                ...
                {  
                    ...
                    if(printOut)System.out.println ("nameNum: " + nameNum);
                    ...
                    if(read)
                    {
                        ...
                        for(int j = 0; j < ext.obsABC.size(); j++)
                        {
                            ...
                            if(obs.equals(title))
                            {
                                ...
                                parValuesWeights.put(title, ext.obsABCWeights.get(j));
                            }
                        }
                    }
                    ...
                    if(read && stats == lastCellNum - 1)
                    {
                        ...
                        obsABCValuesWeights.put((String)ext.villagesNames.get(m), parValuesWeights);
                    }
                }
            }
    ...
    public void obsNamesTranslation()
    {
        ...
        for(int j = 0; j < ext.obsABC.size(); j++)
        {
            ...
            if(obs.equals("seroInc"))obsABCConv.put("seroInc", "Avg seroincidence round 3");

            if(obs.equals("perseroPrevPiglets"))obsABCConv.put("perseroPrevPiglets", "Avg piglets PERseropositivity");
            if(obs.equals("perseroPrevTransition"))obsABCConv.put("perseroPrevTransition", "Avg transition PERseropositivity");
            if(obs.equals("perseroPrevYoungPigs"))obsABCConv.put("perseroPrevYoungPigs", "Avg young pigs PERseropositivity");
            if(obs.equals("perseroPrevAdultPigs"))obsABCConv.put("perseroPrevAdultPigs", "Avg adult pigs PERseropositivity");

            if(obs.equals("THFinalRound"))obsABCConv.put("THFinalRound", "Avg final HT prevalence");
            if(obs.equals("seroInc1"))obsABCConv.put("seroInc1", "Avg seroincidence round 1");
            if(obs.equals("seroInc2"))obsABCConv.put("seroInc2", "Avg seroincidence round 2");
            if(obs.equals("seroInc3"))obsABCConv.put("seroInc3", "Avg seroincidence round 3");
            if(obs.equals("seroInc4"))obsABCConv.put("seroInc4", "Avg seroincidence round 4");
            if(obs.equals("seroInc5"))obsABCConv.put("seroInc5", "Avg seroincidence round 5");
            if(obs.equals("seroInc6"))obsABCConv.put("seroInc6", "Avg seroincidence round 6");
            if(obs.equals("seroInc7"))obsABCConv.put("seroInc7", "Avg seroincidence round 7");
