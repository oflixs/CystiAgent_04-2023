...
public class Extensions implements Serializable
{
    ...
    public static String readPopFromSurveyCensusFile   = "";
    public static String worldInputFile          = "";
    public Boolean printInput                    = true;
    public Boolean indipVillages                 = true;
    ...
    public List<String> villagesNamesJoined = new ArrayList<String>(); 

    public Boolean allVillagesInOneSim = true;
    public String joinedVillageName = "";
    ...
    public List<Double> obsABCWeights = new ArrayList<Double>(); 
    ...
    public void  readInput()
    {
        ...
        String tmp = input.readString("allVillagesInOneSim");
        if(printInput)System.out.println ("Exts Runs ---- " + ": allVillagesInOneSim = " + tmp);
        if(tmp.equals("true"))allVillagesInOneSim = true;
        else if (tmp.equals("false"))allVillagesInOneSim = false;

        if(villagesNames.size() <= 0 && allVillagesInOneSim)
        {
            System.out.println ("Exts Runs ---- with option allVillagesInOneSim simulation");
            System.out.println ("Exts Runs ---- with multiple villages is required");
        ...
        readPopFromSurveyCensusFile = input.readString("readPopFromSurveyCensusFile");
        if(printInput)System.out.println ("Exts Runs ---- " + ": readPopFromSurveyCensusFile = " + readPopFromSurveyCensusFile);
        ...
        if(mode.equals("ABCCalibration") || mode.equals("sensitivityAnalysis"))
        {
            ...
            */
            ...
            obsABCWeights = input.readListDouble("obsABCWeights");

            if(obsABC.size() != obsABCWeights.size())
            {
                System.out.println("Exts Runs ---- in input file obsABC size != obsABCWeights size");
                System.exit(0);
            }
            ...
            for(int i = 0; i < obsABC.size(); i++)
            {
                ...
                Double d = (Double)obsABCWeights.get(i);
                wn = wn + d;
                if(printInput)System.out.println ("Exts Runs ---- " + ": obsABC num " + i + ": " + string + " weight: " + d);
            }

            if(wn == 0.0)
            {
                System.out.println("Exts Runs ---- in input file all obsABCWeights = 0 not allowed");
                System.exit(0);
            }


            List<Double> tmp2 = new ArrayList<Double>(obsABCWeights); 
            for(int i = 0; i < tmp2.size(); i++)
            {
                Double d = (Double)tmp2.get(i);
                d = d/wn;
                obsABCWeights.set(i, d);
            }
    ...
    public void  readWorldInput()
    {
        ...
        String srun = input.readString("indipVillages");
        if(printInput)System.out.println ("Exts Runs ---- " + ": indipVillages = " + srun);
        if(srun.equals("true"))indipVillages = true;
    }
