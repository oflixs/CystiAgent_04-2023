...
//----------------------------------------------------
public class DoAnalysisOutcomePoolJoined implements Serializable
{
    ...
    public String villageNameJoined = "";
    ...
    public Boolean writeInterSims = false;
    public HSSFWorkbook simulatedWorkbook = null;
    public String simSheetName = "simulated stats";
    public Sheet simSheet = null;
    public Boolean firstSimsWrite = true;
    public String simulatedOutFile = "";
    public List<Integer> simIndex  = new ArrayList<Integer>();

    //====================================================
    public DoAnalysisOutcomePoolJoined(Extensions pext, OutcomesPool poutPool)
    {
        ext = pext;
        outPool = poutPool;
        ...
        simulatedWorkbook = new HSSFWorkbook();
    }
    ...
    public void analysis()
    {
        ...
        int numVilla = ext.villagesNamesJoined.size();
        ...
        for (int j = 0; j < numVilla; j++)
        {
            ...
            villageName = ext.villagesNamesJoined.get(j);
            villageNameJoined = ext.villagesNames.get(0);
    ...
    public void calcAverages()
    {
        ...
        String fileName = outPool.parametersInputFiles.get(villageNameJoined);
        System.out.println ("fileName: " + fileName);
        writeInput(workbook, villageNameJoined, fileName);


        if(ext.cystiHumans)
        {
            fileName = outPool.parametersInputFilesCystiHumans.get(villageNameJoined);
            writeInputCystiHumans(workbook, villageNameJoined, fileName);
        }
        ...
        if(writeInterSims)writeToVillagesWorkbookInterventions();
    ...
    public void readFiles(String dir, String what)
    {
        ...
        dataObs = new ArrayList<List<Object>>();
        //dataVertical = new ArrayList<List<Double>>();
        avgData = new ArrayList<Double>();
        dataOut = new ArrayList<Double>();

        avgVertical = new ArrayList<Double>();
        SDVertical = new ArrayList<Double>();
        StVertical = new ArrayList<Double>();

        dataAvgHumanPrevalence    = new ArrayList<Double>();
        dataAvgPigLightPrevalence = new ArrayList<Double>();
        dataAvgPigHeavyPrevalence    = new ArrayList<Double>();

        avgHumanPrevalence = 0.0;
        avgPigLightPrevalence = 0.0;
        avgPigHeavyPrevalence = 0.0;

        sdHumanPrevalence = 0.0;
        sdPigLightPrevalence = 0.0;
        sdPigHeavyPrevalence = 0.0;

        stErrorHumanPrevalence = 0.0;
        stErrorPigLightPrevalence = 0.0;
        stErrorPigHeavyPrevalence = 0.0;
    ...
    public void readFile(String dir, String rFile, String what)
    {
        ...
        try{
            ...
            if(what.equals("seroInc") && sheet == null)return;
            else writeInterSims = true;
...
rows:             
            ...
            //if(what.equals("seroInc"))System.out.println (dataF);
    ...
    public void writeAverages(String what) 
    {
        ...
        if(printOut)System.out.println("================================================");
        if(printOut)System.out.println ("Outp An ---- Writing " + what + "  averages to file");
        if(printOut)System.out.println(" ");
        ...
        if(printOut)System.out.println("titles: " + titles);
        ...
        if(what.equals("Averages"))
        {
            ...
            if(writeInterSims)
            {
                if(firstSimsWrite)
                {
                    firstSimsWrite = false;
                    initSimulatedWorkBook();
                }
                writeSimulatedData();
            }
        }
    ...
    //====================================================
    public void writeToVillagesWorkbookInterventions()
    {
        //System.out.println("Outp An ---- writing to villages workbook           ");
        Sheet sheet = workbook.getSheet("seroInc");

        Sheet villagesSheet = villagesWorkbook.getSheet("seroInc");

        if(villagesSheet == null)villagesSheet = villagesWorkbook.createSheet("seroInc");

        villagesWorkbook.setSheetOrder("seroInc", 0);
...
rows:             
    ...
    public void writeSDPrevalences()
    {
        ...
        if(printOut)System.out.println("================================================");
        if(printOut)System.out.println ("Outp An ---- Writing SDs to file");
        if(printOut)System.out.println(" ");
        ...
        if(printOut)System.out.println("titles size: " + titles.size());
        ...
        if(printOut)System.out.println("lastRow: " + lastRow);
        ...
        for(int i = 0; i < len; i++)
        {
            ...
            for(int j = 0; j < lenLine; j++)
            {
                ...
                else if(objData instanceof Double)
                {
                    ...
                    Integer tmpInt = Integer.valueOf(tmpNorm.get(j));
    ...
    //====================================================
    public void initSimulatedWorkBook()
    {
        simSheet = simulatedWorkbook.getSheet(simSheetName);

        if(simSheet == null)simSheet = simulatedWorkbook.createSheet(simSheetName);

        simulatedWorkbook.setSheetOrder(simSheetName, 0);

        Cell cell = null;
        int cellnum = 0;

        int lastRow = simSheet.getLastRowNum();

        //System.out.println("Last row:" + lastRow);
        //lastRow++;

        //-------------------------------------------------
        Row row = simSheet.createRow(lastRow++);
        cellnum = 0;
        ...
        for(int j = 0; j < titles.size(); j++) 
        {
            ...
            if(titles.get(j).equals("Avg seroincidence round 1"))
            {
                simIndex.add(j);
                cell = row.createCell(cellnum++);
                cell.setCellValue((String)"simSeroInc1");
            }
            else if(titles.get(j).equals("Avg seroincidence round 2"))
            {
                simIndex.add(j);
                cell = row.createCell(cellnum++);
                cell.setCellValue((String)"simSeroInc2");
            }
            else if(titles.get(j).equals("Avg seroincidence round 3"))
            {
                simIndex.add(j);
                cell = row.createCell(cellnum++);
                cell.setCellValue((String)"simSeroInc3");
            }
            else if(titles.get(j).equals("Avg seroincidence round 4"))
            {
                simIndex.add(j);
                cell = row.createCell(cellnum++);
                cell.setCellValue((String)"simSeroInc4");
            }
            else if(titles.get(j).equals("Avg seroincidence round 5"))
            {
                simIndex.add(j);
                cell = row.createCell(cellnum++);
                cell.setCellValue((String)"simSeroInc5");
            }
            else if(titles.get(j).equals("Avg seroincidence round 6"))
            {
                simIndex.add(j);
                cell = row.createCell(cellnum++);
                cell.setCellValue((String)"simSeroInc6");
            }
            else if(titles.get(j).equals("Avg seroincidence round 7"))
            {
                simIndex.add(j);
                cell = row.createCell(cellnum++);
                cell.setCellValue((String)"simSeroInc7");
            }
            else if(titles.get(j).equals("Avg final HT prevalence"))
            {
                simIndex.add(j);
                cell = row.createCell(cellnum++);
                cell.setCellValue((String)"simHTFinalRound");
            }
        }
    ...
    public void writeSimulatedData()
    {
        ...
        {
                List<Object> line_tmp = data.get(i);

                row = simSheet.createRow(lastRow++);
                cellnum = 0;

                cell = row.createCell(cellnum++);
                cell.setCellValue((String)villageName);

                for(int j = 0; j < simIndex.size(); j++) 
                {
                    //System.out.println("Writing sim data: " + j  + " " + titles.get(simIndex.get(j)) + ": " + line_tmp.get(simIndex.get(j)));
                    cell = row.createCell(cellnum++);
                    cell.setCellValue((Double)line_tmp.get(simIndex.get(j)));
                }
    ...
    //====================================================
    public void writeSimulatedToFile()
    {
        String dir = "";
        if(ext.ABC)
        {
            dir = outPool.getabcOne().getABCTimeOutputsDir(); 
            simulatedOutFile = dir + outPool.getabcOne().getABCTime()  +"_InterventionsSimData.xls";
        }
        else 
        {
            dir = "../outputs/" + ext.simName + "/";
            simulatedOutFile = dir + ext.simName  +"_InterventionsSimData.xls";
        }


        System.out.println(" out file " + simulatedOutFile);

        try {

            FileOutputStream out = 
                new FileOutputStream(new File(simulatedOutFile));
            simulatedWorkbook.write(out);
            out.close();
            System.out.println(" output spreadsheet of simulated data for interventions stats written sucessfully.");
