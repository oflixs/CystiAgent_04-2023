...
//----------------------------------------------------
public class InterventionsTest implements Steppable

{
    ...
    double rangeLimit = 100.0;

    //====================================================
    public InterventionsTest(SimState pstate)
    {
        ...
        if(sim.villageDataset.equals("TTEMP"))sim.pigsGenR01.readInterventionsArms();
        ...
        Collections.sort(houses, new HotspotScoreComparator());


        //print some output ------------------------
        System.out.println("Range limit: " + rangeLimit);
        System.out.println("Tot num households in the village: " + sim.householdsBag.size());
        for(int i = 0; i < houses.size(); i++)
        {
            Household hh = (Household)houses.get(i);

            System.out.println("Household: " + hh.shpId + " hotspotScore: " + hh.hotspotScore + " num houses in Range: " + hh.numHHInRange);
        }


        List<Household> tHouses = new ArrayList<Household>();

        for(int t = 0; t < 3; t++)
        {
            Household hh = (Household)houses.get(t);
            Point pHome = (Point)hh.mGeometry.getGeometry();
            for(int i = 0; i < houses.size(); i++)
            {
                Household hh1 = (Household)houses.get(i);
                Point pHome1 = (Point)hh1.mGeometry.getGeometry();
                if(pHome.distance(pHome1) <= rangeLimit)
                {
                    hh1.targeted = true;
                    if(!tHouses.contains(hh1))tHouses.add(hh1);
                }
            }
        }


        System.out.println("Num houses targeted: " + tHouses.size() + " prop. targeted houses: " + (double)tHouses.size()/(double)sim.householdsBag.size());
...
rows:             
    ...
    public void getFinalHTandPC()
    {
        ...
        for(int i = 0; i < sim.humansBag.size(); i++)
        {
            ...
            if(h.age > (int)Math.round(2 * sim.weeksInAYear))
...
rows:             
            ...
            if(sim.extendedOutput)System.out.println(sim.villageName + " R01 final round HT prevalence num humans: " + numHumans + " num infected humans: " + numInfected);
    ...
    public void processHousesFinal()
    {
        ...
        for(int houseNum : housesLinesMap.keySet())
        {
            ...
            if(!line.get(11).equals("."))
            {
                ...
                if(tmp != hh.sshhValue)if(sim.extendedOutput)System.out.println("household " + hId + " differs in sshhValue  original: " + hh.sshhValue + " new: " + tmp);
            }

            if(!line.get(12).equals("."))
            {
                tmp = Integer.parseInt(line.get(12));
                if(tmp != hh.eliminahecValue)if(sim.extendedOutput)System.out.println("household " + hId + " differs in eliminahec  original: " + hh.eliminahecValue + " new: " + tmp);
            }
...
rows:             
    ...
    //====================================================
    public void hotspotsHumanTreatment()
    {
        if(sim.extendedOutput)System.out.println(sim.villageName + " ");
        if(sim.extendedOutput)System.out.println(sim.villageName + " =========================================");
        if(sim.extendedOutput)System.out.println(sim.villageName + " Humans test treatment week: " + sim.numWeeks);
        //System.exit(0);
    ...
    //====================================================
    public void getParamsHotspots()
    {


        for(int i = 0; i < sim.householdsBag.size(); i++)
        {
            Household hhi = (Household)sim.householdsBag.get(i);

            hhi.numHHInRange = 0;

            Point pHomei = (Point)hhi.mGeometry.getGeometry();

            for(int j = 0; j < sim.householdsBag.size(); j++)
            {
                Household hhj = (Household)sim.householdsBag.get(j);

                Point pHomej = (Point)hhj.mGeometry.getGeometry();

                if(pHomei.distance(pHomej) <= rangeLimit)
                {
                    hhi.numHHInRange++;

                    if(!hhj.latrineUsers)hhi.hotspotScore = hhi.hotspotScore + 1.0 * hhj.humans.size();
                    if(!hhj.corralUse.equals("never"))hhi.hotspotScore = hhi.hotspotScore + 1.0 * hhj.pigs.size();
                    if(!hhj.corralUse.equals("sometime"))hhi.hotspotScore = hhi.hotspotScore + sim.propCorralSometimes * hhj.pigs.size();
                }
            }
        }
    }
