...
public class WriteXlsOutput implements Steppable
{
    ...
    public void writeTimeAveragesCysts()
    {
        ...
        //pigs seroprevalences for seroConvMethod 4monthsPrev
        cell = row.createCell(cellnum++);
        cell.setCellValue((String)"Avg piglets PERseropositivity");

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)"Avg transition PERseropositivity");

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)"Avg young pigs PERseropositivity");

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)"Avg adult pigs PERseropositivity");

        if(sim.doInterventions)
        {
            for(int i = 1; i < (sim.seroNRounds + 1); i++)
            {
                cell = row.createCell(cellnum++);
                cell.setCellValue((String)"Avg pigs seropositivity round " + i);

                cell = row.createCell(cellnum++);
                cell.setCellValue((String)"Avg piglets seropositivity round " + i);

                cell = row.createCell(cellnum++);
                cell.setCellValue((String)"Avg seroincidence round " + i);
            }

            cell = row.createCell(cellnum++);
            cell.setCellValue((String)"Avg final HT prevalence");

            cell = row.createCell(cellnum++);
            cell.setCellValue((String)"Avg intervention affect pigs");

            cell = row.createCell(cellnum++);
            cell.setCellValue((String)"Avg intervention affect piglets");

            cell = row.createCell(cellnum++);
            cell.setCellValue((String)"Avg intervention affect seroincidence");

        }

        /*
        cell = row.createCell(cellnum++);
        cell.setCellValue((String)"Avg pig seropositivity nPrint");

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)"Avg piglets seropositivity nPrint");

        cell = row.createCell(cellnum++);
        cell.setCellValue((String)"Avg pig seroincidence nPrint");
        */
        ...
        cell = row.createCell(cellnum++);
        cell.setCellValue(
                sim.seroPrevalencePiglets/(double)sim.seroPrevalencePigletsNorm
                );

        cell = row.createCell(cellnum++);
        cell.setCellValue(
                sim.seroPrevalencePigs/(double)sim.seroPrevalencePigsNorm
                );

        double tmp3 = 0.0;
        if(sim.doInterventions && sim.interventionsType.equals("TTEMP"))
        {
            double sstats = 0;
            for(int rround = 2; rround <= sim.seroNRounds; rround++)
            {
                tmp3 = tmp3 + sim.avgInterSeroIncidencePigsRounds.get(rround)/(double)sim.interventionRepetitions;
                sstats++;
            }
            tmp3 = tmp3/sstats;
        }
        ...
        {
            tmp3 = sim.seroIncidencePigs/(double)sim.seroIncidencePigsNorm;
        }
        ...
        cell = row.createCell(cellnum++);
        cell.setCellValue(
                sim.perseroPrevalencePiglets/(double)sim.perseroPrevalencePigletsNorm
                );

        cell = row.createCell(cellnum++);
        cell.setCellValue(
                sim.perseroPrevalenceAgeTransition/(double)sim.perseroPrevalenceAgeTransitionNorm
                );

        cell = row.createCell(cellnum++);
        cell.setCellValue(
                sim.perseroPrevalencePigsYoung/(double)sim.perseroPrevalencePigsYoungNorm
                );

        cell = row.createCell(cellnum++);
        cell.setCellValue(
                sim.perseroPrevalencePigsAdult/(double)sim.perseroPrevalencePigsAdultNorm
                );
        ...
        if(sim.doInterventions)
        {
            for(int i = 1; i < (sim.seroNRounds + 1); i++)
            {
                cell = row.createCell(cellnum++);
                cell.setCellValue(
                        sim.avgInterSeroPrevalencePigsRounds.get(i)/(double)sim.interventionRepetitions
                        );

                cell = row.createCell(cellnum++);
                cell.setCellValue(
                        sim.avgInterSeroPrevalencePigletsRounds.get(i)/(double)sim.interventionRepetitions
                        );

                cell = row.createCell(cellnum++);
                cell.setCellValue(
                        sim.avgInterSeroIncidencePigsRounds.get(i)/(double)sim.interventionRepetitions
                        );
            }

            cell = row.createCell(cellnum++);
            cell.setCellValue(
                    sim.avgSimPrevHTFinal/(double)sim.interventionRepetitions
                    );

            cell = row.createCell(cellnum++);
            cell.setCellValue(
                        (sim.avgInterSeroPrevalencePigsRounds.get(1) - sim.avgInterSeroPrevalencePigsRounds.get(sim.seroNRounds))/(double)sim.interventionRepetitions
                    );

            cell = row.createCell(cellnum++);
            cell.setCellValue(
                        (sim.avgInterSeroPrevalencePigletsRounds.get(1) - sim.avgInterSeroPrevalencePigletsRounds.get(sim.seroNRounds))/(double)sim.interventionRepetitions
                    );
        ...
        cell = row.createCell(cellnum++);
        cell.setCellValue(
                sim.pigsnPrint/(double)sim.seroincnPrintNorm
                );

        cell = row.createCell(cellnum++);
        cell.setCellValue(
                sim.pigletsnPrint/(double)sim.seroincnPrintNorm
                );

        cell = row.createCell(cellnum++);
        cell.setCellValue(
                sim.seroincnPrint/(double)sim.seroincnPrintNorm
                );
                */
